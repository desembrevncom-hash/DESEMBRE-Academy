...
export const academyAdminEnrollmentApi = {
  ...
  async listEnrollments(
    ...
    const { data, error } = await getSupabaseBrowserClient()!.rpc("admin_list_academy_enrollments", {
  ...
  async approveEnrollment(enrollmentId: string): Promise<boolean> {
    const { data, error } = await getSupabaseBrowserClient()!.rpc("admin_approve_academy_enrollment", {
  ...
  async rejectEnrollment(enrollmentId: string, reason: string): Promise<boolean> {
    const { data, error } = await getSupabaseBrowserClient()!.rpc("admin_reject_academy_enrollment", {
      p_enrollment_id: enrollmentId,
      p_reason: reason,
    });
  ...
  async listStudents(
    ...
    const { data, error } = await getSupabaseBrowserClient()!.rpc("admin_list_academy_students", {
      p_status: status || null,
      p_search: search || null,
    });

    if (error) {
      console.error("Error fetching academy students:", error.message, error.details, error.hint, error.code);
      throw error;
    }
   ...
   */
  async updateStudentStatus(studentId: string, status: string): Promise<boolean> {
    const { data, error } = await getSupabaseBrowserClient()!.rpc("admin_update_academy_student_status", {
      p_student_id: studentId,
      p_status: status,
    });

    if (error) {
      console.error("Error updating student status:", error.message, error.details, error.hint, error.code);
      throw error;
    }
  ...
  async getStudentDetails(studentId: string): Promise<any> {
    const { data, error } = await getSupabaseBrowserClient()!.rpc("admin_get_academy_student_details", {
      p_student_id: studentId,
    });
  ...
  async assignCourseToStudent(studentId: string, courseId: string): Promise<boolean> {
    const { data, error } = await getSupabaseBrowserClient()!.rpc("admin_assign_academy_course_to_student", {
      p_student_id: studentId,
