...
function LessonPlayer() {
  ...
  const flat = useMemo(() => course.modules.flatMap((m) => m.lessons.map((l) => ({ moduleId: m.id, lesson: l }))), [course]);
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        {course.modules.map((m, mi) => (
          <div key={m.id}>
            <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground mb-2">Module {mi + 1}: {m.title}</div>
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <Sheet>
          <SheetTrigger asChild>
            <Button variant="outline" className="lg:hidden rounded-full"><List className="h-4 w-4" /> Chương trình</Button>
          </SheetTrigger>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border/70 bg-card p-4">
            ...
            <div className="flex gap-2">
              <Button
                ...
                disabled={!prev}
                onClick={() => prev && navigate({ to: "/student/courses/$slug/lessons/$lessonId", params: { slug: course.slug, lessonId: prev.lesson.id } })}
              >
              ...
              <Button
                ...
                disabled={!next}
                onClick={() => next && navigate({ to: "/student/courses/$slug/lessons/$lessonId", params: { slug: course.slug, lessonId: next.lesson.id } })}
              >
        ...
        <aside className="hidden lg:block sticky top-6 h-fit rounded-3xl border border-border/70 bg-card p-5 max-h-[calc(100dvh-3rem)] overflow-y-auto">
          ...
          <div className="mt-4 rounded-xl bg-accent/60 p-3 text-xs text-muted-foreground flex items-center gap-2">
            <Lock className="h-3.5 w-3.5" /> Một số bài có thể bị khóa cho đến khi bạn hoàn thành bài trước.
          </div>
        </aside>
      </div>
    </div>
