...
import { academyAdminEnrollmentApi, AcademyStudentAccount } from "@/features/admin/services/academyAdminEnrollmentApi";
import { Search, BookOpen, Loader2, CheckCircle, XCircle } from "lucide-react";
import { toast } from "sonner";
import { getStudentDisplayName, maskEmail, maskPhone } from "@/utils/privacy";
...
function AcademyStudentsAdmin() {
  ...
  const handleApprove = async (id: string) => {
    ...
    setProcessingId(id);
    try {
      await academyAdminEnrollmentApi.updateStudentStatus(id, "active");
  ...
  const handleReject = async (id: string) => {
    if (!confirm("Are you sure you want to reject this student account?")) return;
    setProcessingId(id);
    try {
      await academyAdminEnrollmentApi.updateStudentStatus(id, "blocked");
              ...
              {loading ? (
                ...
                students.map((student) => (
                  <tr key={student.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-slate-900">{student.email ? maskEmail(student.email) : "-"}</div>
                      <div className="text-slate-500 text-xs mt-1">{student.phone ? maskPhone(student.phone) : "-"}</div>
                    </td>
                    <td className="px-6 py-4">
                      <span
