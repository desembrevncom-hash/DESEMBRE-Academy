-- ====================================================================
-- P3C.65H — Auto Create Pending Payment Order on Paid Registration
-- File: scratch/p3c65h_auto_create_paid_orders_rpc.sql
-- ====================================================================

-- 1. DROP existing function signatures
...
-- 2. CREATE updated public_submit_course_registration RPC with auto-order creation for paid courses
...
DECLARE
  ...
  v_course_rec public.courses%ROWTYPE;
  v_course_title text;
  v_course_name text;
  v_clean_phone text;
  v_phone_e164 text;
...
BEGIN
  ...
  -- Compute normalized phone numbers
  IF v_clean_phone LIKE '84%' THEN
    v_phone_e164 := '+' || v_clean_phone;
    v_phone_local := '0' || substr(v_clean_phone, 3);
  ELSIF v_clean_phone LIKE '0%' THEN
    v_phone_e164 := '+84' || substr(v_clean_phone, 2);
    v_phone_local := v_clean_phone;
  ELSE
    v_phone_e164 := '+84' || v_clean_phone;
    v_phone_local := '0' || v_clean_phone;
  END IF;
  ...
  SELECT * INTO v_course_rec FROM public.courses WHERE id = v_batch.course_id;

  -- Fetch course title & clean "Chuyên đề:" prefix
  v_course_title := v_course_rec.title;
  ...
  BEGIN
    ...
    -- 9. Check if course is paid and auto-create academy_orders with pending_payment
    IF v_course_rec.id IS NOT NULL THEN
      IF v_course_rec.deposit_amount IS NOT NULL AND v_course_rec.deposit_amount > 0 THEN
        v_effective_amount := v_course_rec.deposit_amount;
      ELSIF v_course_rec.price_amount IS NOT NULL AND v_course_rec.price_amount > 0 THEN
        v_effective_amount := v_course_rec.price_amount;
      END IF;

      IF v_course_rec.pricing_model = 'paid' OR v_effective_amount > 0 THEN
        INSERT INTO public.academy_orders (
          ...
          NULLIF(trim(p_email), ''),
          v_effective_amount,
          COALESCE(v_course_rec.price_currency, 'VND'),
          'bank_transfer',
          'pending_payment',
          'DESEMBRE ' || v_phone_local,
          v_course_rec.payment_note,
          now(),
          now()
        )
        ...
        v_order_json := jsonb_build_object(
          ...
          'amount', v_effective_amount,
          'currency', COALESCE(v_course_rec.price_currency, 'VND'),
          'payment_status', 'pending_payment',
          'bank_transfer_content', 'DESEMBRE ' || v_phone_local,
          'payment_note', v_course_rec.payment_note
        );
      END IF;
    END IF;

    -- 10. Queue ZNS outbox job
...
GRANT EXECUTE ON FUNCTION public.public_submit_course_registration(uuid, text, text, text, text, text, text, text, text, text) TO anon, authenticated, service_role, postgres;
