-- P3C.32E — Public Training RPC Normalize Uppercase Registration Status
-- File: scratch/p3c32e_fix_public_schedule_uppercase_status.sql
...
BEGIN
  ...
  WHERE LOWER(TRIM(COALESCE(b.registration_status, b.status, 'draft'))) IN ('open', 'published', 'upcoming', 'ongoing');
...
-- Verification Query:
select
  jsonb_array_length(public.public_get_training_schedule()) as batch_count,
  public.public_get_training_schedule() as data;
